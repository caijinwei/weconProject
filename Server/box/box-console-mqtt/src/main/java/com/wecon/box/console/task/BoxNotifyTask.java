...
public class BoxNotifyTask extends Thread {
	...
	private synchronized void updatePlcCfgHandle() {
		try {
			...
			if (null != plcExtendLst) {
				...
				Map<String, List<Map>> groupPlcExtends = GroupOp.groupCfgByMachineCode(
						Converter.convertListOjToMap(plcExtendLst), PlcExtend.UPDATE_PLC_FIELD_FILTER);
	...
	private synchronized void updateAlarmCfgHandle() {
		try {
			...
			if (null != alarmCfgExtendList) {
				Map<String, List<Map>> groupAlarmCfg = GroupOp.groupCfgByMachineCode(
						Converter.convertListOjToMap(alarmCfgExtendList), AlarmCfgExtend.UPDATE_ALARM_FIELD_FILTER);
	...
	private synchronized void deleteAllCfgHandle() {
		try {
			...
			if (null != alarmCfgExtendList) {
				deleteRealHisCfgPublish(Converter.convertListOjToMap(alarmCfgExtendList),
						Constant.DataType.DATA_TYPE_ALARM);
			}
		} catch (Exception e) {
		...
		try {
			...
			if (null != plcExtendLst) {
				Map<String, List<Map>> groupPlcExtends = GroupOp
						.groupCfgByMachineCode(Converter.convertListOjToMap(plcExtendLst), "com", "upd_time");
				if (null != groupPlcExtends) {
					for (Map.Entry<String, List<Map>> entry : groupPlcExtends.entrySet()) {
						// 每个机器码分页进行下发，一页50条
						List<List<Map>> cfgPage = getCfgListByPage(entry.getValue());
		...
		try {
			...
			case ACT_UPDATE_PLC_CONFIG: // 更新通讯口配置反馈
				...
				if (null != updComList && updComList.size() > 0) {
					...
					/*
					 * List<Long> plcIds =
					 * plcInfoApi.getDeleteIdsByUpdTime(getFeedbackDelArgs(
					 * updComList, "com", null)); if(null != plcIds){ for(Long
					 * id : plcIds){ for(Map.Entry<Long, PlcExtend> entry :
					 * plcCfgCache.entrySet()){ PlcExtend p = entry.getValue();
					 * if(id == p.plc_id && p.state !=
					 * Constant.State.STATE_UPDATE_CONFIG_PD){
					 * plcIds.remove(id); } } }
			...
			case ACT_UPDATE_REAL_HISTORY_CONFIG: // 更新实时和历史监控点配置
				...
				RedisManager.publish(ConstKey.REDIS_GROUP_NAME, baseMsgFeedback.getMachine_code(),
						baseMsgFeedback.getMachine_code());
				break;
			...
			case ACT_DELETE_MONITOR_CONFIG: // 删除监控点配置
				...
				List<Long> alarmCfgIds = alarmCfgApi.getDeleteIdsByUpdTime(
						getFeedbackDelArgs(delCfgList, "addr_id", Constant.DataType.DATA_TYPE_ALARM));
				List<Long> realCfgIds = realHisCfgApi.getDeleteIdsByUpdTime(
						getFeedbackDelArgs(delCfgList, "addr_id", Constant.DataType.DATA_TYPE_REAL));
				List<Long> hisCfgIds = realHisCfgApi.getDeleteIdsByUpdTime(
						getFeedbackDelArgs(delCfgList, "addr_id", Constant.DataType.DATA_TYPE_HISTORY));
				...
				// 更新失败的状态
				realHisCfgApi
						.batchUpdateState(getFeedbackFailArgs(delCfgList, "addr_id", Constant.DataType.DATA_TYPE_REAL));
				realHisCfgApi.batchUpdateState(
						getFeedbackFailArgs(delCfgList, "addr_id", Constant.DataType.DATA_TYPE_HISTORY));
				alarmCfgApi.batchUpdateState(
						getFeedbackFailArgs(delCfgList, "addr_id", Constant.DataType.DATA_TYPE_ALARM));
				break;
			...
			for (Map m : updCfgList) {
				try {
					if (UPD_STATE_SUCCESS == Integer.parseInt(m.get("upd_state").toString())) {
						updArgList.add(new String[] { Constant.State.STATE_SYNCED_BOX + "", m.get(idKey).toString(),
								m.get("upd_time").toString() });
					}
				} catch (Exception e) {
					e.printStackTrace();
				}
