...
public class RedisUpdDeviceCfg {
	...
	public boolean pubDelAlarmCfg(long alarmCfgId, long currentPlcId) {
		...
		if (currentPlcId > 0) {
			PlcInfo plcInfo = plcInfoApi.getPlcInfo(currentPlcId);
			device = deviceApi.getDevice(plcInfo.device_id);
		} else {
			AlarmCfg alarmCfg = alarmCfgApi.getAlarmcfg(alarmCfgId);
			device = deviceApi.getDevice(alarmCfg.device_id);
		}
		if (device.state == 1) {
	...
	public boolean pubDelRealHisCfg(long realHisCfgId, long currentPlcId) {
		...
		if (currentPlcId > 0) {
			...
			RealHisCfg realHisCfg = realHisCfgApi.getRealHisCfg(realHisCfgId);
			device = deviceApi.getDevice(realHisCfg.device_id);
		}
		if (null != device && device.state == 1) {
