...
public class RealHisCfgImpl implements RealHisCfgApi {
	...
	private final String SEL_COL = "r.id,r.data_id,r.account_id,r.plc_id,r.name,r.addr,r.addr_type,r.describe,r.digit_count,r.data_limit,r.his_cycle,r.data_type,r.state,r.create_date,r.update_date,r.bind_state,r.device_id,r.rid,r.digit_binary,r.ext_unit,r.dead_set";
		...
		jdbcTemplate.update(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(Connection con) throws SQLException {
				PreparedStatement preState = con.prepareStatement(
						"insert into real_his_cfg(data_id,account_id,plc_id,`name`,addr,addr_type,`describe`,digit_count,data_limit,his_cycle,data_type,state,create_date,update_date,bind_state,device_id,rid,digit_binary,ext_unit,dead_set)values(?,?,?,?,?,?,?,?,?,?,?,?,current_timestamp(),current_timestamp(),1,?,?,?,?,?);",
				...
				preState.setString(17, model.dead_set);
				return preState;
			}
		}, key);
		// 从主键持有者中获得主键值
	...
	public boolean updateRealHisCfg(RealHisCfg model) {
		String sql = "update real_his_cfg SET data_id=?,account_id=?,plc_id=?,`name`=?,addr=?,addr_type=?,`describe`=?,digit_count=?,data_limit=?,his_cycle=?,data_type=?,state=?,update_date=current_timestamp(),bind_state=?,device_id=?,rid=?,digit_binary=?,ext_unit=?,dead_set=? where id=?";
	...
	public List<RealHisCfgDevice> getRealHisCfg(RealHisCfgFilter filter) {
		...
		if (!CommonUtils.isNullOrEmpty(filter.dead_set)) {
			condition.append(" and r.dead_set = ? ");
			params.add(filter.dead_set);
	...
	public static final class DefaultRealHisCfgRowMapper implements RowMapper<RealHisCfg> {
		...
		public RealHisCfg mapRow(ResultSet rs, int i) throws SQLException {
			...
			model.dead_set = rs.getString("dead_set");
	...
	public List<RealHisCfg> findRealHisCfgs(RealHisConfigFilter filter) {
		...
		if (CommonUtils.isNotNull(filter.dead_set)) {
			params.add(filter.dead_set);
			condition.append(" and dead_set = ?");
		}
		sql += condition;
