...
public class TaskThreadPool {
	...
	private TaskThreadPool() {
		workers = new PoolWorker[8];
		for (int i = 0; i < workers.length; i++) {
			workers[i] = new PoolWorker(i);
		}
	}

	private TaskThreadPool(int pool_worker_num) {
	...
	public static TaskThreadPool getInstance2() {
		if (instance == null) {
			synchronized (TaskThreadPool.class) {
				if (instance == null) {
					instance = new TaskThreadPool(5);
				}
			}
		}
		return instance;
			...
			while (isRunning) {
				...
				if (task != null) {
					...
					try {
							...
							System.out.println("Worker<" + index + "> start execute Task<" + task.getTaskId() + ">");
							if (task.getBeginExceuteTime().getTime() - task.getSubmitTime().getTime() > 1000)
								System.out.println("longer waiting time. " + ",<" + index + ">,time:"
										+ (task.getFinishTime().getTime() - task.getBeginExceuteTime().getTime()));*/
...
//						}
							...
							System.out.println("Worker<" + index + "> finish task<" + task.getTaskId() + ">");
							if (task.getFinishTime().getTime() - task.getBeginExceuteTime().getTime() > 1000)
								System.out.println("longer execution time. "  + ",<" + index + ">,time:"
										+ (task.getFinishTime().getTime() - task.getBeginExceuteTime().getTime()));*/
