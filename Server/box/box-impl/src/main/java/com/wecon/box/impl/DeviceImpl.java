    ...
    public List<Map<String, Object>> getDevicesByGroup(long acc_id, int selAlarm) {
        ...
        if(1 == selAlarm){
            ...
            List<String> deviceIdLst = jdbcTemplate.query(
                    "SELECT ac.device_id FROM alarm_cfg ac, alarm_cfg_data acd where ac.alarmcfg_id=acd.alarm_cfg_id and acd.state=1 and ac.device_id in("+sb.substring(1)+")", new RowMapper() {
                        @Override
                        public Object mapRow(ResultSet rs, int i) throws SQLException {
