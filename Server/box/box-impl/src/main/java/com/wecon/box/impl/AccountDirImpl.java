...
public class AccountDirImpl implements AccountDirApi {
    ...
    public long copyAddAccountDir(final AccountDir model) {
        // 相同用户，相同类型，不允许重名
        String sql = "select id  from account_dir where account_id = ? and `name` = ? and `type` = ? and `device_id` = ?  ";
        Map<String, Object> ret = null;
        try {
            ret = jdbcTemplate.queryForMap(sql, new Object[]{model.account_id, model.name, model.type, model.device_id});
        } catch (Exception e) {
            ret = null;
            System.out.println("获取（相同用户，相同类型的分组）为空！（即可创建分组）");
        }
        if (ret != null && ret.get("id") != null) {
